...
do
    ...
    then
        diff "$rom-disassembled.txt" <(./nessemble $rom.rom --disassemble $flags 2>&1) &>/dev/null
        disassembly_fail=$?
    ...
    then
        diff "$rom-simulated.txt" <(./nessemble --simulate $rom.rom --recipe $rom-recipe.txt $flags 2>&1) &>/dev/null
        simulation_fail=$?
